...
import { getGlobalSystemBalances } from "../../lib/queries";
import { TransactionType, Currency } from "@prisma/client";

// Mock sub-components imported below for clean reading
...
export default async function DashboardOverview() {
    ...
    // Fetch balances
    const userBalances = await db.userBalance.findMany({ where: { userId: currentUserId } });
    const userBalancesMap = new Map(userBalances.map(b => [b.currency, b.balance.toNumber()]));

    const globalBalances = await getGlobalSystemBalances();
        ...
        <div className="space-y-8">
            ...
            {role === "Admin" || role === "Mod" ? (
                            ...
                            return (
                                <div key={currency} className="rounded-xl border border-hw-border bg-hw-surface p-5">
                                    <p className="text-sm font-medium text-hw-text-secondary flex items-center">
                                        ...
                                        <p className="inline-block mb-2"> {currency} رصيد الحساب</p>
                                    </p>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        {/* Direct personal client balance read */}
                        {Object.values(Currency).map((currency) => {
            ...
            {/* ZONE 2: Split Action Workspaces based on permissions */}
            <div className="grid grid-cols-1 gap-8 xl:grid-cols-3">

                {/* Main ledger statements display column (Spans wider for admins/mods) */}
                <div className={role !== "Member" ? "xl:col-span-2 space-y-4" : "xl:col-span-3 space-y-4"}>
                    ...
                    {role !== "Member" ? (
                        // Operators see every transactional record logged to audit the ledger
                        <DynamicLedgerTable
                        ...
                        // Clients are sandbox-locked into viewing only their personal ledger lines
                        <DynamicLedgerTable
                            transactions={(await db.transaction.findMany({
                ...
                {/* Right contextual sidebar column: Mounted ONLY if user holds administrative manual entry permissions */}
                {role !== "Member" && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-medium text-hw-text">سجل العمليات</h3>
                        <div className="rounded-xl border border-hw-border bg-hw-surface p-6">
                            {/* Client form handles stateful entry validation; passes data safely down to server actions */}
                            <ManualTransactionForm
                                users={await db.user.findMany({ where: { isActive: true } })}
